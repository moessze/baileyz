...
export const makeSocket = (config) => {
        ...
        for (let i = 0; i <= MAX_RETRIES; i++) {
            try {
                const result = await promiseTimeout(timeoutMs, async (resolve, reject) => {
                    const result = waitForMessage(node.attrs.id, timeoutMs).catch(reject);
                    sendNode(node)
                ...
                if (statusCode && statusCode >= 500 && statusCode < 600 && i < MAX_RETRIES) {
                    const delay = Math.pow(2, i) * 1000;
                    logger.warn({ statusCode, attempt: i + 1 }, `Query failed with 5xx error, retrying in ${delay}ms`);
    ...
    let forceAutoReconnect = true;
    const socketEndHandlers = [];
    /** log & process any unexpected errors */
    const onUnexpectedError = (err, msg) => {
        logger.error({ err }, `unexpected error in '${msg}'`);
    };
    const expectDisconnect = () => {
        forceAutoReconnect = false;
        expectedDisconnect = true;
    };
    const resetConnection = () => {
        forceAutoReconnect = true;
        end(new Boom('Connection reset', { statusCode: DisconnectReason.connectionClosed }));
    };
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    if (retries < MAX_STARTUP_RETRIES && (statusCode === DisconnectReason.connectionLost || statusCode === DisconnectReason.timedOut)) {
                        retries++;
                        const delay = Math.pow(2, retries) * 1000;
                        logger.warn({ err, attempt: retries }, `Network error during connection validation, retrying in ${delay}ms`);
                        await new Promise(resolve => setTimeout(resolve, delay));
                        continue;
                    }
                    throw err;
