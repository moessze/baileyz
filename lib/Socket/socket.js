...
export const makeSocket = (config) => {
            ...
            try {
                await Promise.race([
                    (async () => {
                        for (let retryCount = 0; retryCount <= 3; retryCount++) {
                            try {
                                ...
                                logger.error({ uploadError: uploadError.toString(), count, retryCount }, 'Failed to upload pre-keys to server');
                                if (retryCount >= 3) {
                                    throw uploadError;
                                }
                    ...
                    new Promise((_, reject) => {
                        timeout = setTimeout(() => reject(new Boom('Pre-key upload timeout', { statusCode: 408 })), UPLOAD_TIMEOUT);
                    })
                ]);
            }
            finally {
    ...
    const end = async (error) => {
        ...
        qrTimer = undefined;
        const endHandlers = socketEndHandlers.splice(0);
    ...
    const keepAliveGraceMs = Math.max(10000, connectTimeoutMs || 0);
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        if (!lastDateRecv) {
            lastDateRecv = new Date();
        }
        const diff = Date.now() - lastDateRecv.getTime();
        // Multi-session workloads can delay ping/pong processing under load.
        // Require a wider silence window and more consecutive misses before ending.
        if (diff > keepAliveIntervalMs + keepAliveGraceMs) {
            keepAliveMisses++;
            logger.warn({ diff, misses: keepAliveMisses }, 'keep alive stale');
            if (keepAliveMisses >= 3) {
