...
export const makeSocket = (config) => {
    ...
    /** Wrapper for query with automatic retry on disconnect */
    const safeQuery = async (node, timeoutMs) => {
        const retryLimit = 3;
        for (let attempt = 1; attempt <= retryLimit; attempt++) {
            try {
                return await query(node, timeoutMs);
            } catch (error) {
                if (attempt === retryLimit || error.name === 'Boom' && error.output.statusCode !== 503) {
                    throw error;
                }
                logger?.warn?.({ msgId: node.attrs.id, attempt }, 'query failed, retrying after delay');
    ...
    const end = async (error) => {
        ...
        // --- Improved reconnection backoff logic ---
        const isTransient = error?.output?.statusCode === DisconnectReason.connectionLost || 
                            error?.output?.statusCode === DisconnectReason.connectionClosed;
        
        if (isTransient && connectionAttempts < 5) {
            connectionAttempts++;
            const delayTime = Math.min(1000 * Math.pow(2, connectionAttempts), 30000);
            logger.warn({ attempt: connectionAttempts, delay: delayTime }, 'Connection lost, attempting reconnection with backoff');
            
            setTimeout(() => {
                ws.connect();
            }, delayTime);
            return; // Don't fully close yet, attempt reconnect
        }
        // --------------------------------------------
        ...
        const sendPing = async () => {
            ...
            if (diff > keepAliveIntervalMs + 5000) {
                ...
                try {
                    ...
                    errorCount++;
                    logger.error({ trace: err.stack, errorCount }, 'error in sending keep alive');
                    if (errorCount >= 3) {
                        logger.warn('Too many keepalive failures, forcing reconnect');
                        void end(new Boom('Keepalive failed', { statusCode: DisconnectReason.connectionLost }));
                        return;
                    }
                }
            } else {
            ...
            const interval = keepAliveIntervalMs + Math.random() * 10000;
            keepAliveReq = setTimeout(sendPing, interval);
        };
        keepAliveReq = setTimeout(sendPing, keepAliveIntervalMs);
    };
    ...
    ws.on('open', async () => {
        try {
            /* if (!creds.me?.id) {
                logger.info('Pairing not completed, skipping connection validation');
                return;
             } */
