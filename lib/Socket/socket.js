    ...
    const onWhatsApp = async (...phoneNumber) => {
        ...
        for (const jid of phoneNumber) {
            if (isLidUser(jid)) {
                ...
                if (!contactEnabled) {
                    contactEnabled = true;
                    usyncQuery = usyncQuery.withContactProtocol().withBusinessProtocol().withDisappearingModeProtocol().withUsernameProtocol();
                }
        ...
        if (results) {
            return results.list.filter(a => a.contact !== undefined).map(({ contact, id }) => ({ jid: id, exists: contact }));
        }
    };
