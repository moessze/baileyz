...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, bytesToCrockford, configureSuccessfulPairing, Curve, DEF_MEDIA_HOST, derivePairingCodeKey, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
     ...
     */
    const inFlightQueryTags = new Set();
    /**
    ...
    const uploadPreKeys = async (count = MIN_PREKEY_COUNT) => {
        ...
        const uploadLogic = async (retryCount) => {
            ...
            try {
                ...
                // server has the keys.
                if (pendingCommit)
                    ev.emit('creds.update', pendingCommit);
            }
            catch (uploadError) {
