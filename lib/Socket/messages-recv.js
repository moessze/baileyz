...
export const extractLinkCodeCompanionRegBuffers = (node) => {
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const identityChangeCleanupTimers = new Map()
    ...
    const handleMessage = async (node) => {
        ...
        let acked = false;
        try {
            if (node.tag === 'status' && !node.attrs.from?.includes('@newsletter')) {
                await sendMessageAck(node);
                return;
            }
            const { fullMessage: msg, category, author, decrypt } = decryptMessageNode(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger);
            if (node.tag === 'status') {
                msg.isNewsletterStatus = true;
            }
                ...
                if (ackFrom && !inFlight463Recoveries.has(ackFrom)) {
                    ...
                    void (async () => {
                        try {
                            ...
                            const issueJid = await resolveIssuanceJid(ackFrom, sock.serverProps.lidTrustedTokenIssueToLid, getLIDForPN, getPNForLID, logger)
    ...
    ws.on('CB:status', async (node) => {
        await processNode('status', node, 'processing status', handleMessage);
    });
