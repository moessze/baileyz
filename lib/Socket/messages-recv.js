...
export const makeMessagesRecvSocket = (config) => {
        ...
        if (enableAutoSessionRecreation && messageRetryManager && retryCount > 1) {
            try {
                ...
                if (shouldRecreateSession) {
                    ...
                    logger.debug({ fromJid }, 'Successfully deleted session, now expecting new prekey query');
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        // Helper to attempt encryption and handle re-keying
        const encryptAndSend = async (msg) => {
            try {
                // Assuming you have an encryptMessage function or similar available in this context
                // You might need to adjust this call to match your actual encryption function name
                return await encryptMessage({ jid: participant, data: msg });
            } catch (error) {
                if (error.message.includes('session') || error.message.includes('MAC')) {
                    logger.warn({ error, participant }, 'Enkripsi gagal, mencoba re-keying otomatis...');
                    
                    // 1. Hapus sesi agar dipaksa buat baru
                    await authState.keys.set({ session: { [sessionId]: null } });
                    
                    // 2. Fetch PreKey baru
                    await fetchAndSaveNewPreKey(participant, { query, signalRepository }); 
                    
                    // 3. Coba kirim ulang pesan
                    return await encryptMessage({ jid: participant, data: msg });
                }
                throw error;
            }
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        // Participant list hash validation to ensure cache consistency.
        if (attrs.phash) {
            logger.info({ attrs }, 'received phash mismatch in ack, invalidating cache and resending message...');
            // Invalidate cache
            if (isJidGroup(attrs.from)) {
                await cachedGroupMetadata?.(attrs.from, true);
            } else {
                userDevicesCache.del(jidNormalizedUser(attrs.from));
            }
