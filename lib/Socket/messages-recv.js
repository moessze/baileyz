...
import { isTcTokenExpired, resolveTcTokenJid, storeTcTokensFromIqResult } from '../Utils/tc-token-utils.js';
import { areJidsSameUser, binaryNodeToString, getAllBinaryNodeChildren, getBinaryNodeChild, getBinaryNodeChildBuffer, getBinaryNodeChildren, getBinaryNodeChildString, isJidGroup, isJidNewsletter, isJidStatusBroadcast, isLidUser, isPnUser, jidDecode, jidNormalizedUser, JidWithDevice, S_WHATSAPP_NET } from '../WABinary/index.js';
import { extractGroupMetadata } from './groups.js';
import { makeMessagesSocket } from './messages-send.js';
export const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, getMessage, shouldIgnoreJid, enableAutoSessionRecreation } = config;
    const sock = makeMessagesSocket(config);
    const { ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, getPrivacyTokens } = sock;
                ...
                try {
                    ...
                    if (senderTs !== null && senderTs !== undefined && !isTcTokenExpired(senderTs)) {
                        ...
                        getPrivacyTokens([normalizedJid], senderTs)
                            .then(result => storeTcTokensFromIqResult({
                            result,
                            fallbackJid: tcJid,
                            keys: authState.keys,
                            getLIDForPN,
    ...
    const handleNotification = async (node) => {
        ...
        try {
            ...
            sendMessageAck(node).catch(ackErr => logger.error({ ackErr }, 'failed to ack notification'))
        }
    };
