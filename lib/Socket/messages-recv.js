...
export const makeMessagesRecvSocket = (config) => {
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        if (messageRetryManager) {
            await messageRetryManager.semaphore?.runExclusive(async () => {
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            try {
                ...
                if (authorJid) await signalRepository.prefetchSessionsForUser(authorJid);
            } catch (err) {
                logger.warn({ err }, 'failed to prefetch sessions for user');
            }
