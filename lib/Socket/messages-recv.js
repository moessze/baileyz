...
export const makeMessagesRecvSocket = (config) => {
    ...
    const fetchMessageHistory = async (count, oldestMsgKey, oldestMsgTimestamp) => {
        ...
        const pdoMessage = {
            historySyncOnDemandRequest: {
                ...
                oldestMsgTimestampMs: (typeof oldestMsgTimestamp === 'number' || typeof oldestMsgTimestamp === 'string') && Number(oldestMsgTimestamp) > 9999999999 ? Math.floor(Number(oldestMsgTimestamp) / 1000) : Number(oldestMsgTimestamp),
    ...
    const handleGroupNotification = (fullNode, child, msg) => {
        ...
        if (isLidUser(actingParticipantLid) && isPnUser(actingParticipantPn)) {
            mappings.push({ lid: actingParticipantLid, pn: actingParticipantPn });
        }
        if (isLidUser(affectedParticipantLid) && isPnUser(affectedParticipantPn)) {
            mappings.push({ lid: affectedParticipantLid, pn: affectedParticipantPn });
        }
        if (mappings.length > 0) {
            signalRepository.lidMapping.storeLIDPNMappings(mappings).catch(e => logger?.warn({ err: e }, 'Failed to store LID mapping'));
        }
        switch (child?.tag) {
            case 'create':
            ...
            case 'leave':
                ...
                if (lidMappings.length > 0) {
                    signalRepository.lidMapping.storeLIDPNMappings(lidMappings).catch(e => logger?.warn({ err: e }, 'Failed to store LID mappings'));
                }
            ...
            case 'revoked_membership_requests':
                ...
                if (isLidUser(affectedParticipantLid) && isPnUser(affectedParticipantPn)) {
                    signalRepository.lidMapping.storeLIDPNMappings([{ lid: affectedParticipantLid, pn: affectedParticipantPn }]).catch(e => logger?.warn({ err: e }, 'Failed to store LID mapping'));
                }
                msg.messageStubType = WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD;
                msg.messageStubParameters = [
                    JSON.stringify({ lid: affectedParticipantLid, pn: affectedParticipantPn }),
