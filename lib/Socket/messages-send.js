...
export const resolveMessageSendJid = async (jid, getLIDForPN, getPNForLID) => {
    if (isLidUser(jid) || isHostedLidUser(jid)) {
        let additionalAttributes = undefined;
        let remoteJidAlt = undefined;
        if (getPNForLID) {
            const pn = await getPNForLID(jid);
            if (pn) {
                remoteJidAlt = jidNormalizedUser(pn);
                additionalAttributes = {
                    addressing_mode: 'lid',
                    peer_recipient_pn: remoteJidAlt
                };
            }
        }
        return { jid, remoteJidAlt, addressingMode: 'lid', additionalAttributes };
    }
    ...
    const lid = getLIDForPN ? await getLIDForPN(jid) : undefined;
...
export const makeMessagesSocket = (config) => {
    ...
    const recentlyChangedIdentities = new Set()
    ...
    const sendPeerDataOperationMessage = async (pdoMessage) => {
        ...
        const msgId = await relayMessage(meJid, protocolMessage, {
            additionalAttributes: {
                category: 'peer',
                ...(pdoMessage.peerDataOperationRequestType === proto.Message.PeerDataOperationRequestType.HISTORY_SYNC_ON_DEMAND ? { privacy_sensitive: '1' } : { push_priority: 'high_force' })
            },
            additionalNodes: [
                {
                    tag: 'meta',
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        const resolved = await resolveMessageSendJid(jid, getLIDForPN, getPNForLID);
        jid = resolved.jid;
        if (resolved.additionalAttributes) {
            additionalAttributes = { ...additionalAttributes, ...resolved.additionalAttributes };
        }
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                const resolvedJid = await resolveMessageSendJid(jid, getLIDForPN, getPNForLID);
