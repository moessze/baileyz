...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, generateWAMessageFromContent, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds, delay, DEF_MEDIA_HOST } from '../Utils/index.js';
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                const fullMsg = await generateWAMessage(jid, content, {
                    ...
                    getProfilePicUrl: async (jid, type, timeout) => {
                        const cacheKey = `${jid}:${type || 'preview'}`
                        const cached = profilePictureCache.get(cacheKey);
                        if (cached !== undefined) {
                            if (cached === null) throw new Error('Profile picture not found (cached)');
                            return cached;
                        }
                        try {
                            const url = await sock.profilePictureUrl(jid, type, timeout);
                            profilePictureCache.set(cacheKey, url);
                            return url;
                        } catch (err) {
                            profilePictureCache.set(cacheKey, null);
                            throw err;
                        }
                    },
