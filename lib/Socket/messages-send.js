...
import { areJidsSameUser, ensureLID, getBinaryNodeChild, getBinaryNodeChildren, isHostedLidUser, isHostedPnUser, isJidBot, isJidGroup, isJidMetaAI, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, PSA_WID, S_WHATSAPP_NET } from '../WABinary/index.js';
import { USyncQuery, USyncUser } from '../WAUSync/index.js';
import { makeNewsletterSocket } from './newsletter.js';

export const resolveMessageSendJid = async (jid, getStoredLIDForPN) => {
...
export const makeMessagesSocket = (config) => {
    ...
    const messageSendMutex = makeMutex();
    let mediaConn;
    ...
    const relayMessage = async (jid, message, options) => {
        return await messageSendMutex.mutex(async () => {
            const { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList } = options;
            ...
            const finalJid = jid;
            let msgIdLocal = msgId || generateMessageIDV2(meId);
            let useUserDevicesCacheLocal = useUserDevicesCache !== false;
            let useCachedGroupMetadataLocal = useCachedGroupMetadata !== false && !isStatus;
            const participants = [];
            ...
            const result = await (isGroupOrStatus ? 
                groupParticipantsQuery(jid, statusJidList) : 
                (useUserDevicesCacheLocal ? fetchUserDevices([jid], true) : fetchUserDevices([jid], false))
            );
            const { nodes: encryptedNodes, shouldIncludeDeviceIdentity: shouldIncludeDeviceIdentity_ } = await encryptMessage(jid, message, {
                participants: result,
                additionalAttributes,
                additionalNodes,
                msgId: msgIdLocal,
            ...
            shouldIncludeDeviceIdentity = shouldIncludeDeviceIdentity_;
            const node = {
                tag: 'message',
                attrs: {
                    id: msgIdLocal,
                    type: 'text',
                    to: jid,
                    ...(isGroupOrStatus ? { participant: participant?.jid } : {})
                },
                content: [
                    ...binaryNodeContent,
                    ...encryptedNodes
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                let lidJid = await ensureLID(jid, getLIDForPN);
                if (isPnUser(lidJid) || isHostedPnUser(lidJid)) {
                    logger.debug({ jid }, 'LID for PN not found, fetching user info');
                    ...
                    // Simplified parsing - need to ensure this matches the expected response structure
                    // For now, if info is obtained, update mapping.
                    // This is a minimal implementation to attempt fetching.
                    // In a production scenario, robust parsing of the usync result is required.
                }
                const resolvedJid = await resolveMessageSendJid(lidJid, getStoredLIDForPN);
                    ...
                    // Cache profile picture URL fetches
                    getProfilePicUrl: async (jid, type) => {
                        const cacheKey = `${jid}:${type}`;
                        const cached = config.mediaCache?.get(cacheKey);
                        if (cached) return cached;
                        const url = await sock.profilePictureUrl(jid, type);
                        if (url) config.mediaCache?.set(cacheKey, url);
                        return url;
                    },
                ...
                const additionalAttributes = { ...(resolvedJid.additionalAttributes || {}) };
