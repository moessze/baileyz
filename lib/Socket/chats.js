...
export const makeChatsSocket = (config) => {
    ...
    // Timeout for Syncing state to detect infinite loops
    let syncingStateTimeout;
    const HISTORY_SYNC_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes

    const stopHistorySync = () => {
        if (syncState === SyncState.Syncing) {
            logger.warn('Forcing history sync to stop and transitioning to Online state');
            clearTimeout(syncingStateTimeout);
            syncingStateTimeout = undefined;
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        if (historyMsg && syncState === SyncState.AwaitingInitialSync) {
            ...
            if (shouldProcessHistoryMsg) {
                ...
                syncingStateTimeout = setTimeout(() => {
                    if (syncState === SyncState.Syncing) {
                        logger.warn('History sync timed out, forcing stop');
                        stopHistorySync();
                    }
