...
export const makeCommunitiesSocket = (config) => {
    ...
    const { authState, ev, query, upsertMessage, signalRepository, safeQuery } = sock;
    const communityQuery = async (jid, type, content) => safeQuery({
        tag: 'iq',
        attrs: {
            type,
            xmlns: 'w:g2',
    ...
    const communityMetadata = async (jid) => {
        ...
        const { metadata, lidMappings } = extractCommunityMetadata(result);
    ...
    const communityFetchAllParticipating = async () => {
        ...
        if (communitiesChild) {
            ...
            for (const communityNode of communities) {
                const { metadata: meta, lidMappings } = extractCommunityMetadata({
                    tag: 'result',
                    attrs: {},
