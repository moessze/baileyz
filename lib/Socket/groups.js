...
export const makeGroupsSocket = (config) => {
    ...
    const { authState, ev, query, upsertMessage, signalRepository, getLIDForPN } = sock;
    ...
    const groupMetadata = async (jid) => {
        const tcTokenContent = await buildTcTokenFromJid({ authState, jid, getLIDForPN });
        const result = await groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' }, content: tcTokenContent }]);
        const { metadata, lidMappings } = extractGroupMetadata(result);
        if (lidMappings.length) {
            await signalRepository.lidMapping.storeLIDPNMappings(lidMappings);
        }
        return metadata;
    };
    const groupFetchAllParticipating = async () => {
        ...
        if (groupsChild) {
            ...
            const allLidMappings = [];
            for (const groupNode of groups) {
                const { metadata: meta, lidMappings } = extractGroupMetadata({
                    tag: 'result',
                    attrs: {},
                    content: [groupNode]
                });
                data[meta.id] = meta;
                allLidMappings.push(...lidMappings);
            }
            if (allLidMappings.length) {
                await signalRepository.lidMapping.storeLIDPNMappings(allLidMappings);
            }
        }
...
export const extractGroupMetadata = (result) => {
    ...
    const lidMappings = metadata.participants
        .filter(p => p.lid && p.phoneNumber)
        .map(p => ({ lid: p.lid, pn: p.phoneNumber }));
    return { metadata, lidMappings };
};
