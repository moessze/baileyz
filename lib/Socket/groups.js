...
export const makeGroupsSocket = (config) => {
    ...
    const groupMetadata = async (jid) => {
        ...
        const lidMappings = meta.participants
            .filter(p => p.lid && p.phoneNumber)
            .map(p => ({ lid: p.lid, pn: p.phoneNumber }));
        if (lidMappings.length > 0) {
            sock.signalRepository.lidMapping.storeLIDPNMappings(lidMappings).catch(e => sock.logger?.warn({ err: e }, 'Failed to store LID mappings'));
        }
        return meta;
    };
            ...
            for (const groupNode of groups) {
                ...
                for (const p of meta.participants) {
                    if (p.lid && p.phoneNumber) {
                        lidMappings.push({ lid: p.lid, pn: p.phoneNumber });
                    }
                }
            }
            if (lidMappings.length > 0) {
