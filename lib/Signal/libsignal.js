...
import { GroupCipher, GroupSessionBuilder, hasLogger, ProtocolAddress, SenderKeyDistributionMessage, SenderKeyName, SenderKeyRecord, SessionBuilder, SessionCipher, SessionRecord, setLogger } from 'whatsapp-rust-bridge'
        ...
        async encryptGroupMessage({ group, meId, data }) {
            return parsedKeys.transaction(async () => {
                const { skdm } = await ensureSenderKeyAndCreateSkdm(group, meId)
				    ...
				    const ciphertext = await new GroupCipher(storage, group, meAddr).encrypt(data)
