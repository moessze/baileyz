...
import { decryptPreKeyWithSnapshot, decryptWhisperWithSnapshot, encryptWithSnapshot, GroupCipher, GroupSessionBuilder, hasLogger, importLegacySessionRecordV1, processBundleWithSnapshot, projectLegacySenderKeyRecordV1, projectLegacySessionRecordV1, ProtocolAddress, SenderKeyDistributionMessage, SenderKeyName, SessionRecord, setLogger } from 'whatsapp-rust-bridge';
import { proto } from '../../WAProto/index.js';
import { generateSignalPubKey } from '../Utils/crypto.js';
import { MISSING_KEYS_ERROR_TEXT } from '../Utils/decode-wa-message.js';
import { isHostedLidUser, isHostedPnUser, isLidUser, isPnUser, jidDecode, jidEncode, transferDevice, WAJIDDomains } from '../WABinary/index.js';
import { hasOpenLegacySession, isLegacySessionEntry, isLegacySessionRecord, legacySessionInfo } from './legacy-session.js';
import { fromTypedRecord, toTypedRecord } from './legacy-session-codec.js';
 ...
 */
function preKeyIdsFrom(ciphertext) {
    try {
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
     ...
     * one.
     */
    const withSession = async (jid, run, extra) => {
     ...
     */
    const toStoredSession = (wireJid, session) => {
        const projection = projectLegacySessionRecordV1(session);
        if (projection.status !== 'projected') {
            logger.warn({ wireJid, issue: projection.issue }, 'session not expressible in the legacy shape, storing bridge bytes');
            return session;
        }
        ...
        const overflowing = projection.record.sessions.some(indexed => indexed.session.chains.some(chain => chain.messageKeys.length > MAX_LEGACY_MESSAGE_KEYS));
        if (overflowing) {
            logger.warn({ wireJid }, 'session holds more skipped keys than the legacy shape accepts, storing bridge bytes');
            return session;
        }
        return fromTypedRecord(projection.record);
    };
    /** Pre-WASM records are converted on read; bridge records pass straight through. */
    const readSessionBytes = async (wireJid, stored) => {
        if (!stored)
            return undefined;
        try {
            if (isLegacySessionRecord(stored)) {
                if (!hasOpenLegacySession(stored))
                    return undefined;
     ...
     */
    const applyChanges = async (wireJid, changes) => {
        const update = {};
    ...
    const repository = {
        ...
        async processSenderKeyDistributionMessage({ item, authorJid }) {
            ...
            return parsedKeys.transactWith({ records: [{ type: 'sender-key', id: senderNameStr }] }, async () => builder.process(senderName, senderMsg));
        },
        async decryptMessage({ jid, type, ciphertext }) {
            ...
            const { plaintext } = await withSession(jid, async (snapshot, address) => {
                try {
                    let out;
                    if (type === 'pkmsg') {
                        try {
        ...
        async encryptMessage({ jid, data }) {
            return withSession(jid, async (snapshot, address) => {
                ...
                return {
                    type: out.messageType === 3 ? 'pkmsg' : 'msg',
        ...
        async migrateSession(fromJid, toJid) {
            ...
            for (const [sessionKey, sessionData] of Object.entries(existingSessions)) {
                if (sessionData) {
                    ...
                    if (already) {
                        ...
                        logger.warn({ device: deviceNum, migrating: takeCandidate ? sessionKey : already.addrStr }, 'device has a session under both the plain and hosted address; migrating one and leaving the other');
                        if (takeCandidate) {
            ...
            const migrationOps = found.map(({ jid, addrStr }) => {
                ...
                const lidWithDevice = jidDecode(jid).device === 99
