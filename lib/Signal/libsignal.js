...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
	...
	const withSessionLocks = async (keys, work) => {
      const sorted = Array.from(new Set(keys)).sort();
      const acquire = async (idx) => {
	...
	const resolveSignalAddress = async (jid) => {
	...
	const resolveSessionKey = async (jid) => (await resolveSignalAddress(jid)).toString();
	...
	const runIsolated = (exec, key) => parsedKeys.isolatedTransaction
      ? parsedKeys.isolatedTransaction(exec)
       : parsedKeys.transaction(exec, key);
