...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const repository = {
        ...
        async prefetchSessionsForUser(jid) {
            ...
            const promises = userDevices.map(async (device) => {
                ...
                const deviceJid = deviceNum === 0 ? `${user}@s.whatsapp.net` : `${user}:${deviceNum}@s.whatsapp.net`;
                await storage.loadSession(jidToSignalProtocolAddress(deviceJid).toString());
            });
