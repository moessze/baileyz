...
const checkCertValidity = (details) => {
    const now = Math.floor(Date.now() / 1000);
    if (details.notBefore && now < details.notBefore) {
        throw new Boom('certificate not valid yet', { statusCode: 400 });
    }
    if (details.notAfter && now > details.notAfter) {
        throw new Boom('certificate expired', { statusCode: 400 });
    }
};
...
export const makeNoiseHandler = ({ keyPair: { private: privateKey, public: publicKey }, NOISE_HEADER, logger, routingInfo }) => {
    ...
    return {
        ...
        processHandshake: ({ serverHello }, noiseKey) => {
            ...
            checkCertValidity(details);
            checkCertValidity(leafDetails);
