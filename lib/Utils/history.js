...
export const processHistoryMessage = async (item, logger) => {
    const messages = [];
    const contacts = [];
    const chats = [];
    const lidPnMappings = [];
        ...
        case proto.HistorySync.HistorySyncType.ON_DEMAND:
            for (const chat of item.conversations) {
                ...
                for (let i = 0; i < msgs.length; i++) {
                    if (i % 50 === 0) {
                        await new Promise(resolve => setImmediate(resolve)); // yield periodically
                    }
                    const item = msgs[i];
...
export const downloadAndProcessHistorySyncNotification = async (msg, options, logger) => {
    ...
    return await processHistoryMessage(historyMsg, logger);
};
