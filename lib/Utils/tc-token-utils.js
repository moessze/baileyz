...
export async function resolveTcTokenJid(jid, getLIDForPN, logger) {
    if (isLidUser(jid))
        return jid;
...
export async function resolveIssuanceJid(jid, issueToLid, getLIDForPN, getPNForLID, logger) {
    if (issueToLid) {
        if (isLidUser(jid))
            return jid;
        if (!isPnUser(jid)) return jid
...
export async function buildTcTokenFromJid({ authState, jid, baseContent = [], getLIDForPN, logger }) {
    try {
...
export async function storeTcTokensFromIqResult({ result, fallbackJid, keys, getLIDForPN, onNewJidStored, logger }) {
