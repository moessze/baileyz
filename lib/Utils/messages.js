...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        // Cache the group invite thumbnail
        if (options.getProfilePicUrl) {
            const cacheKey = `${message.groupInvite.jid}:groupInviteThumbnail`;
            const cachedThumbnail = options.mediaCache?.get(cacheKey);
            if (cachedThumbnail) {
                m.groupInviteMessage.jpegThumbnail = cachedThumbnail;
            } else {
                ...
                if (pfpUrl) {
                    ...
                    if (resp.ok) {
                        ...
                        options.mediaCache?.set(cacheKey, buf);
                    }
                }
            }
        }
