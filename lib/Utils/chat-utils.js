 ...
 */
export const extractSyncdPatches = async (result, options, host) => {
...
export const downloadExternalBlob = async (blob, options, host) => {
...
export const downloadExternalPatch = async (blob, options, host) => {
...
export const decodePatches = async (name, syncds, initial, getAppStateSyncKey, options, minimumVersionNumber, logger, validateMacs = true, host) => {
    const newState = {
        ...initial,
    ...
    for (const syncd of syncds) {
        ...
        if (validateMacs) {
            ...
            if (Buffer.compare(snapshotMac, computedSnapshotMac) !== 0) {
                logger?.warn({ name, version: newState.version }, 'LTHash verification failed — throwing LTHashMismatchError (M7) so caller can resync from snapshot');
