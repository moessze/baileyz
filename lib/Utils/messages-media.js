...
export const downloadContentFromMessage = async ({ mediaKey, directPath, url, fileSha256, fileEncSha256 }, type, opts = {}) => {
    let downloadUrl = opts.mediaUrl || url;
    if (!downloadUrl && directPath) {
        if (!directPath.startsWith('/')) {
            throw new Boom(`media download path does not start with slash: ${directPath}`, { statusCode: 400 });
        }
        downloadUrl = getUrlFromDirectPath(directPath, opts.host ?? DEF_MEDIA_HOST);
    }
    if (!downloadUrl) {
        throw new Boom('No valid directPath present in message', { statusCode: 400 });
    }

    if (!mediaKey && !fileEncSha256) {
        const fetched = await getHttpStream(downloadUrl, opts);
        let expectedHash = fileSha256 ? (typeof fileSha256 === 'string' ? Buffer.from(fileSha256, 'base64') : (Buffer.isBuffer(fileSha256) ? fileSha256 : Buffer.from(fileSha256))) : undefined;
        
        if (!opts.allowNoHash && !expectedHash) {
            expectedHash = Buffer.alloc(32);
        }

        let hasher = undefined;
        if (expectedHash) {
        ...
        const output = new Transform({
            ...
            final(callback) {
                if (hasher) {
                    ...
                    if (expectedHash.length !== 32 || !hash.equals(expectedHash)) {
                        return callback(new Boom('Invalid unencrypted media SHA256', { statusCode: 400 }));
                    }
                }
                callback();
            }
